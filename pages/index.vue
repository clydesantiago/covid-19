<template>
  <div>
    ...
    <v-dialog v-model="dialog" max-width="600px">
      <v-card v-if="result">
        <v-card-title>
          <span class="headline">Last Checked: {{ $moment(result.lastChecked).calendar() }}</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              <v-col v-for="(resultItem, index) in result.covid19Stats" :key="index" cols="12">
                <v-card
                  class="mx-auto"
                  outlined
                >
                  ...
                  <v-card-subtitle>
                    Updated: {{ $moment(resultItem.lastUpdate).calendar() }}
                  </v-card-subtitle>

                  <v-list-item @click="$event.preventDefault();">
                    <v-list-item-avatar>
                      <v-icon>mdi-skull-outline</v-icon>
                    </v-list-item-avatar>
                    <v-list-item-content>
                      <v-list-item-title>{{ resultItem.deaths }}</v-list-item-title>
                      <v-list-item-subtitle>Deaths</v-list-item-subtitle>
                    </v-list-item-content>
                  </v-list-item>
                  ...
                  <v-list-item @click="$event.preventDefault();">
                    ...
                    <v-list-item-content>
                      <v-list-item-title>{{ resultItem.confirmed }}</v-list-item-title>
                  ...
                  <v-list-item @click="$event.preventDefault();">
                    ...
                    <v-list-item-content>
                      <v-list-item-title>{{ resultItem.recovered }}</v-list-item-title>
                      <v-list-item-subtitle>Recovered</v-list-item-subtitle>
                    </v-list-item-content>
                  </v-list-item>
                </v-card>
              </v-col>
    ...
    getData (country) {
      ...
      this.$axios({
        ...
        headers: {
          ...
          'x-rapidapi-key': '58AP51oSHZmsh1ALnNsZZ0PJfm31p1jrsXUjsnJXSj6QYNhG0c'
        },
        params: {
          country
        }
        ...
        .then((response) => {
          this.showDialogResult(response.data.data)
        })
        .catch((error) => {
          console.log(error)
        })
